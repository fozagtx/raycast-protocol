...
import {
    IERC20
} from "@chainlink/contracts-ccip/src/v0.8/vendor/openzeppelin-solidity/v4.8.0/contracts/token/ERC20/IERC20.sol";
...
contract ProgrammableTokenTransfers is CCIPReceiver, OwnerIsCreator {
    ...
    error FailedToWithdrawToken(address token, address beneficiary, uint256 amount);
    ...
    function _sendData(uint64 _destinationChainSelector, address _receiver, bytes memory _data)
        internal
        onlyOwner
        onlyAllowlistedDestinationChain(_destinationChainSelector)
        validateReceiver(_receiver)
        returns (bytes32 messageId)
    {
        ...
        // Emit an event with message details
        emit MessageSent(
            messageId, _destinationChainSelector, _receiver, _data, address(0), 0, address(s_linkToken), fees
        );
    ...
    function withdrawToken(address _beneficiary, address _token) public onlyOwner {
        ...
        bool success = IERC20(_token).transfer(_beneficiary, amount);
        if (!success) {
            revert FailedToWithdrawToken(_token, _beneficiary, amount);
        }
    }
