    ...
    ERC4626 // Make this "is ILogAutomation"
{
    ...
    function _requireVaultUnlocked() internal view {
        if (vaultLocked) {
            revert VaultLocked();
        }
    }
    ...
    // ERC4626 OVERRIDES
    //
    function deposit(uint256 assets, address receiver) public override whenNotLock returns (uint256 shares) {
        shares = super.deposit(assets, receiver);
        _checkDepositThreshold();
    }

    function mint(uint256 shares, address receiver) public override whenNotLock returns (uint256 assets) {
        assets = super.mint(shares, receiver);
        _checkDepositThreshold();
    }

    function initSlowWithdraw(uint256 assets, address receiver, address owner)
        public
        whenNotLock
    ...
    function initSlowRedeem(uint256 shares, address owner) public whenNotLock {
        ...
        if (pendingToRedeem > 0 && lastRequestToRedeemFromDst[owner] <= lastRedeemFromDst) {
        ...
        returns (uint256 assets)
    {
        ...
        if (pendingToRedeem > 0) {
            uint256 diff = (shares >= pendingToRedeem) ? pendingToRedeem : shares;
    ...
    function execute() public {
        ...
        _sendDataAndToken(
            ...
            abi.encodeWithSignature("deposit(uint256)", _depositableAssetToDestination),
            address(asset),
    ...
    function quit() public {
        ...
        uint256 shareRatio = pendingToRedeemFromDst.mulDivDown(10 ** asset.decimals(), totalSupply);

        _sendData(
            destinationChainId,
            destinationSenderReceiver,
            abi.encodeWithSignature("redeem(uint256,uint256)", shareRatio, _currentAsset())
        );
    }
    ...
    function addDestinationChainId(uint64 _destinationChainId) public onlyOwner {
        destinationChainId = _destinationChainId;
    }

    function addDestinationSenderReceiver(address _destinationSenderReceiver) public onlyOwner {
    ...
    function _checkDepositThreshold() internal {
        uint256 _depositableAssetToDestination = depositableAssetToDestination();
        if (_depositableAssetToDestination >= depositThreshold) {
            emit TimeToExecute(_depositableAssetToDestination, depositThreshold);
        }
    }
    ...
    function performUpkeep(bytes calldata performData) external {
        if (keccak256("TimeToExecute(uint256,uint256)") == abi.decode(performData, (bytes32))) {
            execute();
        } else if (keccak256("TimeToQuit(uint256,uint256)") == abi.decode(performData, (bytes32))) {
            quit();
        }
    }
}
