...
contract UnitTests is StdCheats, Test {
    ...
    address public constant DEV_ACCOUNT_2 = 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC;
    ...
    function setUp() external {
        ...
        receiver = new ProgrammableTokenTransfers(address(router), address(linkToken));

        senderReceiver = new SenderReceiver(address(router), address(linkToken));

        sourceVault = new SourceVault(ERC20(address(ccipBnM)), "SourceVault", "SV", address(router), address(linkToken));

        destinationVault = new DestinationVault(ERC20(address(ccipBnM)), "DestinationVault", "DV");
    ...
    function transferLinkTokensToSourceVault() public {
        ...
        assertTrue(linkToken.transfer(address(sourceVault), TOKEN_MINT_BALANCE));
    ...
    function testCorrectAddressesForRouterAndLinkToken() public view {
        ...
        assertTrue(address(router) == sourceVault.getRouterAddress(), "Router address is not correct");
        assertTrue(address(linkToken) == sourceVault.getLinkTokenAddress(), "Link token address is not correct");
    }
    ...
    function testSenderHasTokens() public {
        ...
        assertTrue(linkBalance == TOKEN_MINT_BALANCE, "SourceVault does not have the correct amount of link tokens");
    }
    ...
    function testCorrectSourceVaultAssetAddress() public view {
        assertEq(address(sourceVault.asset()), address(ccipBnM), "Asset address is incorrect");
    }
