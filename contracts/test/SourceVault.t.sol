...
contract SourceVaultTest is UnitTests {
    ...
    function testInitSlowRedeemRevertWithExistingPendingSharesToBeRedeemedFirst() public asDevAccount1 {
        uint256 shares = sourceVault.balanceOf(DEV_ACCOUNT_0);
        ...
        vm.startPrank(DEV_ACCOUNT_1);
        vm.expectRevert(SourceVault.ExistingPendingSharesToBeRedeemedFirst.selector);
        sourceVault.initSlowRedeem(halfShares, DEV_ACCOUNT_0);
    }
    ...
    function testRedeemWithoutPendingToRedeem() public asDevAccount0 {
        ...
        assertEq(sourceVault.isPendingToRedeem(DEV_ACCOUNT_0), 0, "isPendingToRedeem(DEV_ACCOUNT_0) != 0");
        assertEq(sourceVault.totalPendingToRedeem(), 0, "totalPendingToRedeem != 0");
        assertEq(sourceVault.pendingToRedeemFromDst(), 0, "pendingToRedeemFromDst != 0");
        uint256 withdrawalAmount = ccipBnM.balanceOf(DEV_ACCOUNT_0) - initBalance;
    ...
    function testRedeemWithPendingToRedeem() public asDevAccount0 {
        ...
        assertEq(
            sourceVault.isPendingToRedeem(DEV_ACCOUNT_0), halfShares, "isPendingToRedeem(DEV_ACCOUNT_0) != halfShares"
        );
        assertEq(sourceVault.totalPendingToRedeem(), halfShares, "totalPendingToRedeem != halfShares");
        assertEq(sourceVault.pendingToRedeemFromDst(), halfShares, "pendingToRedeemFromDst != halfShares");
        uint256 withdrawalAmount = ccipBnM.balanceOf(DEV_ACCOUNT_0) - initBalance;
        assertEq(withdrawalAmount, halfShares);
    }
    ...
    function testRedeemWithPendingToRedeemAfterQuit() public asDevAccount0 {
        ...
        assertEq(sourceVault.totalPendingToRedeem(), halfShares, "totalPendingToRedeem != halfShares");
        assertEq(sourceVault.pendingToRedeemFromDst(), 0, "pendingToRedeemFromDst != 0");
        assertEq(ccipBnM.balanceOf(DEV_ACCOUNT_0) - initBalance, halfShares, "withdrawal amount != halfShares");
    }
    ...
    function testExecute() public {
        ...
        uint256 senderReceiverBalance = destinationVault.balanceOf(address(senderReceiver));
    ...
    function testQuit() public {
        ...
        assertEq(senderReceiverBalance, 0, "assets on destination chain should be 0");

        assertEq(assetsOfSourceVault, userBalance, "assets on source chain should be equal to user balance");
        assertEq(sourceVault.isPendingToRedeem(DEV_ACCOUNT_0), userBalance, "isPendingToRedeem(DEV_ACCOUNT_0) != 0");
        assertEq(block.timestamp - initialTimestamp, 1, "lastRedeemFromDst is incorrect");
        assertEq(sourceVault.totalAssets(), assetsOfSourceVault, "totalAssets is incorrect");
    }
    ...
    function _initSlowRedeem() internal {
        ...
        assertEq(sourceVault.lastRequestToRedeemFromDst(DEV_ACCOUNT_0), block.timestamp);
    ...
    function _initSlowRedeemTwice() internal {
        ...
        assertEq(sourceVault.isPendingToRedeem(DEV_ACCOUNT_0), shares, "isPendingToRedeem(DEV_ACCOUNT_0) != shares");
        assertEq(sourceVault.totalPendingToRedeem(), shares, "totalPendingToRedeem != shares");
        assertEq(sourceVault.pendingToRedeemFromDst(), shares, "pendingToRedeemFromDst != shares");
    }
